...
| ASI04 Agentic Supply Chain | Malicious skills/hooks/MCP servers (executable code) | `manifest.json` sha256 fingerprints (CI-checked), `hooks/skill-validate.sh` security pass, `mcp-policy.json` defaultDeny + routingContract, branch protection on all changes | policy + CI | Covered; ahead of GitHub Copilot's name-matching-only MCP allowlist. Gap: no signed manifest, no third-party scanner (SkillSpector-style) — see issue #74 |
