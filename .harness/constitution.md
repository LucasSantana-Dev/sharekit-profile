...
| Invariant | Meaning |
|-----------|---------|
| `no-ai-attribution` | Never add AI co-author markers, "Generated with" trailers, or bot attribution to commits, PRs, issues, or release notes. Author of record is the operator. |
...
| `storage-policy` | New repos, clones, worktrees, datasets, model weights, and large caches go on the configured dev-root (`${DEV_ROOT}/`), not the internal disk. Never clone or download large data under `~/` outside tool-config dirs. |
