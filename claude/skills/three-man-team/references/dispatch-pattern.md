...
agent dispatch \
  --name architect \
  --model opus \
  --prompt "Read the <project-a> Discord bot codebase and write a plan for shipping /guild-config command. Details in task. Output to ~/.claude/plans/guild-config.md" \
