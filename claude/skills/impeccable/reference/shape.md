...
Before asking questions, extract what you already know from PRODUCT.md, DESIGN.md, and the codebase. Fill in as many discovery slots as possible.

**From PRODUCT.md**, you can answer:
- Purpose & problem the feature solves (from "Product Purpose")
- Target user role and context (from "Users")
- Tone and anti-references (helps visual direction)
- Register (brand vs product)

**From DESIGN.md**, you can answer:
- Color strategy for this surface (Restrained / Committed / Full palette / Drenched)
- Existing typography system and fonts
- Existing component patterns
- Dark vs light theme default

**From codebase**, you can infer:
- Content data structure (scan existing feature or dummy data)
- Edge cases from error boundaries or validation
- Component reuse patterns
- Known technical constraints

At the start of the interview, summarize what you've pre-filled: *"From your PRODUCT.md, I see this is for [role] trying to [goal]. From DESIGN.md, we're using [color strategy] and [font]. So I only need to ask about [2 remaining unknowns]."* This shortens the interview from 3 rounds to 1-2.

If PRODUCT.md or DESIGN.md is missing or sparse, note it in the brief and ask about the missing pieces during the live interview.

### Interview cadence

After pre-filling from context, you'll know which discovery slots are still empty. Ask only those; aim for 1-2 rounds, not 3.

- Use the harness's structured question tool when one exists. Otherwise, ask directly in chat and stop.
- Ask **2-3 questions per round**, then wait for answers.
- Reference what you pre-filled to avoid redundant questions. *"I saw in DESIGN.md that color strategy is Restrained. Can you confirm that still applies here, or should we shift to Committed?"*
- Focus each round on the largest unknowns: purpose/audience, content/data scope, visual direction and anti-goals, constraints.
- Don't ask three full rounds if context already answers two of them.
...
After synthesizing the brief, present it to the user with a summary of what came from pre-fill vs what came from the interview: *"Design Direction comes from your DESIGN.md (Committed color strategy); Scope and Layout Strategy came from our conversation."* This transparency shows the brief is grounded in project reality, not generated from thin air.
