---
name: impeccable
description: Use when the user wants to design, redesign, shape, critique, audit, polish, distill, harden, optimize, adapt, animate, colorize, extract, or improve a frontend interface. Covers websites, landing pages, dashboards, product UI, components, forms, onboarding, and empty states. Handles UX review, visual hierarchy, accessibility, performance, theming, typography, layout, color, motion, and design systems. Also use for UI audits or bland designs needing boldness. Not for backend-only tasks.
version: 3.1.0
user-invocable: true
