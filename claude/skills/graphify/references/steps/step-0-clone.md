...
Only run if a GitHub URL was given. Skip if user provided a local path.
...
Once `graphify-out/graph.json` exists, the fast path takes over: any codebase question runs `graphify query` directly on the merged graph — no re-extraction, no size gate.
