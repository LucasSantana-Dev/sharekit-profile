...
# --- detect name + slug ---
if [ -z "$NAME" ]; then
  NAME="$(git -C "$PROJ" remote get-url origin 2>/dev/null | sed -E 's#.*[:/]##; s#\.git$##')"
  [ -z "$NAME" ] && NAME="$(basename "$PROJ")"
fi
SLUG="$(printf '%s' "$PROJ" | sed 's#/#-#g')"   # matches ~/.claude/projects/<slug> convention
...
[ -n "$(git -C "$PROJ" rev-parse --is-inside-work-tree 2>/dev/null)" ] && \
