...
description: Flag over-engineering — speculative abstractions, premature generalization, single-use "flexibility", unnecessary indirection, config for things that never vary, and patterns heavier than the project needs. Scope-tightenable: audit a path, the current diff, specific categories, or a severity floor (default narrow, never whole-repo unprompted). Read-only and advisory: proposes the simpler alternative with its cost, never rewrites. Use in review, before merge, or when a module "feels" too clever.
user-invocable: true
