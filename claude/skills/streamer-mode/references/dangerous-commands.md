...
| `git remote -v`, `git config --list` | Remotes of any scheme can embed credentials | `git remote > /tmp/out` (names only); credential check: `git config --get-regexp 'remote\..*\.url' > /tmp/out` then `grep -cE '://[^/@]*:[^/@]*@\|https?://[^/@]*@' /tmp/out` (count only, never print the line). Catches `user:pass@` under any scheme plus token-only userinfo over http(s), while leaving ordinary `ssh://git@host` remotes alone |
...
| `aws configure list`, `aws sts get-caller-identity` | Access key IDs, account ID | Verify auth by exit code ONLY, output discarded: `aws sts get-caller-identity > /dev/null 2>&1 && echo authed`. This is the single documented exception to the Core Rule 1 ban, and it holds only while the redirection is present. Running it unredirected, or piping it to `jq`, prints the account ID and ARN on screen. |
